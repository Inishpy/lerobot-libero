...
class LoRALinear(nn.Module):
    ...
    def forward(self, x):
        # Assume all submodules are already on the correct device/dtype
...
def load_lora_adapter(model, path, strict=True):
    ...
    if missing:
        print(f"[LoRA adapter] Missing keys (expected for adapter): {missing}")
    if unexpected:
        print(f"[LoRA adapter] Unexpected keys (adapter-only): {unexpected}")
...
def replace_linear_with_lora(module, r=8, alpha=16, dropout=0.05, target_modules=None, prefix=''):
    ...
    for name, child in module.named_children():
        ...
        if isinstance(child, nn.Linear) and (target_modules is None or name in target_modules or full_name in target_modules):
            ...
            # Move LoRALinear to same device and dtype as original Linear
            lora_layer = lora_layer.to(device=child.weight.device, dtype=child.weight.dtype)
